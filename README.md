...
실제 송신 전에는 다음 항목을 자동으로 검증합니다.

- FSK 톤이 유효하게 계산되는지 확인합니다.
- root 권한과 설치된 네이티브 백엔드가 있는지 확인합니다.
...
| `homebridge_helper.py` | Homebridge용 최소 권한·경로·명령 검증 helper |
...
| `third_party/librpitx/` | 고정 버전의 최소 fskburst/DMA/GPIO 소스와 GPL-3.0 라이선스 |
...
Raspberry Pi OS Bookworm에서는 `rpitx` 저장소나 `/opt/vc` VideoCore
라이브러리를 별도로 설치하지 않습니다. 이 저장소에 고정 버전으로 포함한
`librpitx`의 `fskburst`와 필요한 DMA/GPIO/mailbox 소스만 정적 라이브러리로
빌드하고, `somfy-rpitx-tx` 송신기 하나에 직접 링크합니다. upstream의 전체 앱,
`install.sh`, `/opt/vc/include`, `/opt/vc/lib`, `libbcm_host`는 사용하지 않습니다.
...
합니다. Raspberry Pi 5의 BCM2712/RP1 구조는 upstream `librpitx`가 지원하지
않으므로 Pi 5를 지원한다고 간주하면 안 됩니다. 실제 송신은 `/dev/mem`과
`/dev/vcio` 접근 때문에 root 권한이 필요합니다.
...
`config.example.json`과 설치 후 `/etc/somfy-rpitx/config.json`의 기본 RF
부분은 다음과 같습니다.
...
3. 이미 등록된 실제 리모컨의 PROG 버튼을 모터가 짧게 움직일 때까지 누릅니다.
4. 모터의 등록 대기 시간 안에 `sudo somfy-rpitx prog`를 한 번 실행합니다.
5. 모터가 다시 짧게 움직이면 `up/down/stop`으로 확인합니다.
...
`sudo make install`이 Homebridge 서비스용 sudo 권한을 자동으로 설치합니다.
Homebridge는 root로 전체 CLI를 실행하지 않고, 경로와 `up/down/stop`을 다시
검증하는 `somfy-rpitx-homebridge` helper만 호출합니다. 사용자가 sudoers를
직접 편집할 필요는 없습니다.
...
RF 설정은 `/etc/somfy-rpitx/config.json`을 사용하며, 가상 리모컨 상태는
고유 ID를 이용해 자동 생성합니다.
...
Bookworm Raspberry Pi에서 vendored 라이브러리와 송신기만 다시 빌드하려면:
...
- [vendored librpitx 기준 커밋](https://github.com/F5OEO/librpitx/tree/f01bdb64bcdb6207f448379193bc0a8accb9aa22)
- [Raspberry Pi: Bullseye에서 Bookworm으로 전환](https://pip.raspberrypi.com/categories/685-whitepapers-app-notes-compliance-guides/documents/RP-006519-WP/Transitioning-from-Bullseye-to-Bookworm.pdf)
